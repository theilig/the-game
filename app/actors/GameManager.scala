...
class GameManager(gameDao: GameDao)
  ...
  override def receive: Receive = {
    ...
    case ConnectToGame(gameId) =>
      val response: Future[Product] = gameDao.findById(gameId).map {
        case Some(gameRow) if !gameRow.finished =>
          val gameActor = context.actorOf(GameActor.props(gameId, gameDao), s"Game$gameId")
...
object GameManager {
  ...
  def props(gameDao: GameDao)(implicit ec: ExecutionContext): Props =
    Props(new GameManager(gameDao))
}
